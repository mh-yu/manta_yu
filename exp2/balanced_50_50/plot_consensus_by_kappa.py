...
AVG_CSV = "consensus_summary_avg.csv"
OUTPUT_DIR = "plots_avg_by_sigma_kappa"
EXCLUDED_SIGMAS = {5}
...
def plot_all_lines(avg_rows):
    ...
    fig.suptitle("Consensus Metrics Averaged Over Runs", fontsize=14)

    cmap = plt.get_cmap("tab10")
    series = sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1]))
    for index, ((sigma, reference), group_rows) in enumerate(series):
        group_rows.sort(key=lambda row: row["kappa"])
        ...
        label = f"sigma={sigma}, reference={reference}"
        color = cmap(index % 10)
    ...
    fig.savefig(os.path.join(OUTPUT_DIR, "consensus_avg_lines_by_sigma_kappa.png"), dpi=180)
    plt.close(fig)
