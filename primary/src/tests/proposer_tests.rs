...
async fn propose_payload() {
    ...
    // Bootstrap round 1 is emitted without payload.
    let header = rx_headers.recv().await.unwrap();
    assert_eq!(header.round, 1);
    assert!(header.payload.is_empty());

    // Unlock round 2 and send enough digests for the payload.
    ...
    tx_parents.send((parents, 1)).await.unwrap();
    ...
    // Ensure the proposer makes a correct follow-up header from the provided payload.
    let header = rx_headers.recv().await.unwrap();
